...
class CalcDisplay(ttk.Frame):
  ...
  #      s.theme_use("alt")
    ...
    def addDigit(self, digito):
        if self._value[0] == "-" and lend(self._value) >= 11 or len(self._value) >= 10:
        #if len(self._value) == 10:
            ## Añado este if, para poner el menos aunque haya 10 digitos en el display
            #if self._value[0] == "-":
            3    self.pintar()
            return
...
class CalcButton(ttk.Frame):
    def __init__(self, parent, **kwargs):
        ...
        if "bw" in kwargs:
            ...
            bw = 1
        """
#Esta espresion de la linea de abajo es lo mismo que el anterior if
#Esto se llama operador ternario
        ...
        self.pack_propagate(0) #(los componenetes hijos no controlan su tamaño)
...
#        self.display.place(x=0, y=0)
        ...
        #utilizamos las fuciones anonimas lambda para poder pasar un parametro por comand
...
# La funcion enviar ya no la llamamos
